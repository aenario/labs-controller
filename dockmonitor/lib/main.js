...
(function() {
  var Client, ControllerClient, appsPath, async, axon, client, controllerUrl, couchUrl, dataSystemUrl, exec, fs, getAuthCouchdb, getHomePort, getToken, handleError, homeClient, homeUrl, indexerUrl, manifest, pkg, program, proxyUrl, spawn, statusClient, token, version, waitInstallComplete;
  ...
  waitInstallComplete = function(slug, callback) {
    var socket, timeoutId;
    axon = require('axon');
  ...
  program.command("install <app> ").description("Install application").option('-r, --repo <repo>', 'Use specific repo').option('-d, --displayName <displayName>', 'Display specific name').action(function(app, options) {
    return getHomePort(function(homeport) {
      var path;
      manifest.name = app;
